...
plotCompFits <- function( rpt, d=16:100, folder="." )
{
  pdf( file=paste(folder,"/compFit.pdf",sep=""), height=11, width=8.5 )

  par( mfrow=c(rpt$nS/2,2), mar=c(0,0,0,0), oma=c(4,5,2,1) )
...
plotMu <- function( rpt, folder="." )
{
  par( mfrow=c(rpt$nS/2,2) )
  for( s in 1:rpt$nS )
  {
    plot(rpt$mu_st[s,])
  }
